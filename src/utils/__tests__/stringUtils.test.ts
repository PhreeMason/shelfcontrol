...
describe('stringUtils', () => {
  describe('toTitleCase', () => {
    ...
    it('should keep lowercase articles and prepositions in the middle', () => {
      ...
      expect(toTitleCase('for whom the bell tolls')).toBe('For Whom the Bell Tolls');
    });
    ...
    it('should capitalize first word even if it is typically lowercase', () => {
      ...
      expect(toTitleCase('and then there were none')).toBe('And Then There Were None');
    });
    ...
    it('should preserve acronyms in all caps', () => {
      expect(toTitleCase('the FBI investigation')).toBe('The Fbi Investigation');
    ...
    it('should handle contractions properly', () => {
      ...
      expect(toTitleCase("it's a wonderful life")).toBe("It's a Wonderful Life");
    ...
    it('should handle hyphenated words', () => {
      ...
      expect(toTitleCase('state-of-the-art technology')).toBe('State-Of-The-Art Technology');
    });

    it('should handle complex mixed cases', () => {
      expect(toTitleCase('HARRY POTTER and the SORCERER\'S STONE')).toBe('HARRY POTTER and the SORCERER\'S STONE');
      expect(toTitleCase('the lord of the rings: the fellowship of the ring')).toBe('The Lord of the Rings: the Fellowship of the Ring');
    });
    ...
    it('should handle numbers and special characters', () => {
      expect(toTitleCase('2001: a space odyssey')).toBe('2001: a Space Odyssey');
      expect(toTitleCase('catch-22 by joseph heller')).toBe('Catch-22 by Joseph Heller');
    });
