...
describe('DeadlinesService', () => {
  ...
  describe('getArchivedDeadlines', () => {
    const userId = 'user-123';

    it('should return filtered archived deadlines', async () => {
      const mockDeadlines = [
        {
          id: 'rd-1',
          book_title: 'Book 1',
          status: [{ status: 'complete', created_at: '2024-01-15T00:00:00Z' }],
        },
        {
          id: 'rd-2',
          book_title: 'Book 2',
          status: [{ status: 'reading', created_at: '2024-01-10T00:00:00Z' }],
        },
        {
          id: 'rd-3',
          book_title: 'Book 3',
          status: [{ status: 'set_aside', created_at: '2024-01-20T00:00:00Z' }],
        },
      ];
      ...
      const result = await deadlinesService.getArchivedDeadlines(userId);

      expect(result).toHaveLength(2);
      expect(result[0].id).toBe('rd-3'); // Most recent completion date
      expect(result[1].id).toBe('rd-1');
    });

    it('should handle deadlines without status array', async () => {
      const mockDeadlines = [
        {
          id: 'rd-1',
          book_title: 'Book 1',
    ...
    it('should throw error when query fails', async () => {
      const mockError = new Error('Archived query failed');
