-- Fix custom date timestamp handling in get_daily_activities
--
-- Problem: Line 225 was casting date-only values to timestamptz incorrectly:
--   dcd.date::timestamptz
--
-- This interpreted the date as UTC midnight, causing it to shift backwards
-- when clients convert UTC to local timezone (e.g., Jan 15 00:00 UTC becomes
-- Jan 14 19:00 in US Eastern time).
--
-- Solution: Use noon UTC instead of midnight to avoid day boundary issues.
-- Any timezone offset (max +/- 14 hours) from noon will stay on the same calendar day.
...
BEGIN
  ...
  -- Custom dates (all-day events like press releases, cover reveals, blog tours)
  -- FIX: Use noon UTC instead of midnight to avoid day boundary issues
  -- when client converts UTC to local timezone
  SELECT
    dcd.date as activity_date,
    'custom_date'::text as activity_type,
    d.id as deadline_id,
    d.book_title,
    ((dcd.date + TIME '12:00:00') AT TIME ZONE 'UTC')::timestamptz as activity_timestamp,
    jsonb_build_object(
...
'Aggregates all user activities (deadlines due, deadlines created, progress updates, status changes, notes, reviews, custom dates) for a date range. Includes previous_progress and previous_status for showing deltas and transitions. Custom dates use noon UTC to avoid timezone day-shift issues. Used by calendar view for performance optimization.';
