-- Create deadline_custom_dates table for storing named dates on deadlines
-- Examples: press releases, cover reveals, blog tours, etc.

create table deadline_custom_dates (
  id text not null primary key default generate_prefixed_id('dcd_'),
...
-- Update get_daily_activities to include custom dates
...
'Aggregates all user activities (deadlines due, deadlines created, progress updates, status changes, notes, reviews, custom dates) for a date range. Includes previous_progress and previous_status for showing deltas and transitions. Used by calendar view for performance optimization.';
