-- Fix get_daily_activities function to remove non-existent 'unit' column
-- The deadlines table does not have a 'unit' column
...
BEGIN
  ...
  -- Progress updates on this date (FIXED: removed 'unit' column reference)
  SELECT
...
'Aggregates all user activities (deadlines due, deadlines created, progress updates, status changes, notes, reviews) for a date range. Used by calendar view for performance optimization. Fixed to remove non-existent unit column.';
